...
for i, chunk in enumerate(text_chunks):
    #print(f"Processing text chunk {i}: {chunk[:100]}...")  # Print first 100 characters of each chunk
    doc = Document(id=f"text_chunk_{i}", page_content=chunk, metadata={"type": "text"})
    docs.append(doc)

vector_store.add_documents(documents=docs)
# Uncomment the following lines to generate embeddings for text, tables, and images
...
results = vector_store.similarity_search(query=query_str, k=1)
for doc in results:
